...
import { addQuizToIndex, removeQuizFromIndex } from '../lib/indexManager';
 ...
 */
export const onQuizApproved = functions
  .region('us-central1')
  .runWith({
    secrets: ['GOOGLE_AI_API_KEY'],
    timeoutSeconds: 120, // Allow time for embedding generation
    memory: '512MB',
  })
  ...
  .onUpdate(async (change, context) => {
    ...
    if (beforeStatus !== 'approved' && afterStatus === 'approved') {
      ...
      try {
        await addQuizToIndex(quizId, afterData);
    ...
    if (beforeStatus === 'approved' && afterStatus === 'approved') {
      ...
      const hasContentChange = importantFields.some(
 ...
 */
export const onQuizCreatedApproved = functions
  .region('us-central1')
  .runWith({
    secrets: ['GOOGLE_AI_API_KEY'],
    timeoutSeconds: 120,
  ...
  .onCreate(async (snapshot, context) => {
    ...
    if (quizData?.status === 'approved') {
      ...
      console.log(`ℹ️ New quiz ${quizId} created with status '${quizData?.status}', not indexing`);
    }
  });
