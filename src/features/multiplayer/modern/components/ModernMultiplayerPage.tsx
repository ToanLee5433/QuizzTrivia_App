...
const ModernMultiplayerPage: React.FC = () => {
  ...
  // Restore room state from URL when page loads
  useEffect(() => {
    const restoreRoom = async () => {
      if (urlRoomId && !isRestoringRoom) {
        console.log('🔄 Restoring room from URL:', urlRoomId);
        setIsRestoringRoom(true);
        try {
          // URL might contain roomCode (6 chars) or roomId (Firebase ID)
          let actualRoomId = urlRoomId;
          
          // If URL param looks like a room code (6 uppercase alphanumeric), query Firestore
          if (/^[A-Z0-9]{6}$/.test(urlRoomId)) {
            console.log('🔍 URL contains room code, querying for roomId...');
            const roomDoc = await modernMultiplayerService.findRoomByCode(urlRoomId);
            if (roomDoc) {
              actualRoomId = roomDoc.id;
              console.log('✅ Found room:', { code: urlRoomId, roomId: actualRoomId });
            } else {
              throw new Error('Room not found with code: ' + urlRoomId);
            }
          }
          
          // Check if service already has this room
          if (modernMultiplayerService.roomId === actualRoomId) {
            console.log('✅ Room already active, switching to lobby view');
            setRoomId(actualRoomId);
            setCurrentView('room-lobby');
          } else {
            // Need to rejoin/restore the room
            console.log('🔄 Attempting to restore room connection...');
            // Manually setup roomId and listeners in service
            modernMultiplayerService.roomId = actualRoomId;
          ...
          showToast({
            type: 'error',
            title: 'Failed to Restore Room',
            message: 'Could not rejoin the room. Returning to quiz selection.',
            duration: 4000
          });
    ...
    restoreRoom();
  }, [urlRoomId, navigate, showToast, isRestoringRoom]);

  const handleRoomCreated = (newRoomId: string, roomCode?: string) => {
    console.log('🎯 Room created, switching to lobby:', { roomId: newRoomId, code: roomCode });
    setRoomId(newRoomId);
    setCurrentView('room-lobby');
    // Navigate to URL with roomCode (6 chars) for cleaner URL
    const urlCode = roomCode || newRoomId;
    navigate(`/multiplayer/${urlCode}`, { replace: true });
    showToast({
      type: 'success',
      title: 'Room Created Successfully!',
      message: `Room code: ${roomCode || newRoomId}`,
      duration: 4000
    });
  };
  ...
  const handleJoinRoom = async (roomCode: string, password?: string) => {
    try {
      ...
      showToast({
        ...
        message: error.message || 'Could not join the room. Please check the code.',
        duration: 4000
      });
    }
  };
