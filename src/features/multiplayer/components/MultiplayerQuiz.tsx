...
const MultiplayerQuiz: React.FC<MultiplayerQuizProps> = ({
  ...
  const [timeLeft, setTimeLeft] = useState<number>(() => gameData?.timeLeft ?? roomData?.settings?.timePerQuestion ?? 30);
  const [showResults, setShowResults] = useState(false);
  const [questionResults, setQuestionResults] = useState<any>(null);
  const [nextQuestionCountdown, setNextQuestionCountdown] = useState<number | null>(null);
  const timerRef = useRef<number | null>(null);

  const question = gameData?.currentQuestion || gameData?.questions?.[gameData?.currentQuestionIndex || 0] || { id: 'q', title: 'Question', options: [] };
  ...
  useEffect(() => {
    ...
    const initial = gameData?.timeLeft ?? roomData?.settings?.timePerQuestion ?? 30;
    setTimeLeft(initial);
  }, [question?.id]);
  ...
  const percent = useMemo(() => {
    const total = roomData?.settings?.timePerQuestion ?? 30;
    return Math.max(0, Math.min(100, Math.round((timeLeft / total) * 100)));
  }, [timeLeft, roomData?.settings?.timePerQuestion]);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              <div className="bg-gradient-to-r from-purple-500 to-blue-600 text-white px-3 sm:px-4 py-2 rounded-xl font-bold text-sm sm:text-base text-center">
                Question {gameData?.index || 1} of {gameData?.total || 10}
              </div>
