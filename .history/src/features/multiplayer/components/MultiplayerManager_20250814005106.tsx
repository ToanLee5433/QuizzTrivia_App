...
const MultiplayerManager: React.FC<MultiplayerManagerProps> = ({
  ...
  useEffect(() => {
    ...
    if (readyCountdown <= 0) {
      // Only host can start game to match Firestore security rules
      if (multiplayerService && state.roomId && state.roomData?.hostId === currentUserId) {
        console.log('🎮 Starting game from countdown (as host)...');
      ...
      } else if (state.roomData?.hostId !== currentUserId) {
            ...
            <div className="flex-1">
              ...
              {process.env.NODE_ENV === 'development' && (
                <div className="mb-4 p-3 bg-yellow-100 border border-yellow-300 rounded">
                  <strong>🐛 Debug:</strong> gameData keys: {state.gameData ? Object.keys(state.gameData).join(', ') : 'No gameData'} | 
                  Questions: {state.gameData?.questions?.length || 0}
                </div>
              )}
              <MultiplayerQuiz
