...
const MultiplayerQuiz: React.FC<MultiplayerQuizProps> = ({
  ...
  const [forceSubmitWarning, setForceSubmitWarning] = useState(false);
  ...
  // Real-time Firestore listener for game synchronization
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(roomRef, (doc) => {
      if (doc.exists()) {
        ...
        // Reset states when question changes via Firestore
        if (data.gameData?.currentQuestionIndex !== realtimeGameData?.currentQuestionIndex) {
          console.log('📝 Question changed via Firestore sync');
          setSelected(null);
          setLocked(false);
        ...
        // Handle results phase and countdown synchronization
  ...
  }, [roomData?.id, realtimeGameData?.currentQuestionIndex]);
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
      // Only auto-submit if player has selected an answer OR after grace period
      if (!locked && selected !== null) {
        handleSubmit();
      } else if (!locked && selected === null) {
        // Give player a warning and a few more seconds to select
        console.warn('⏰ Time up! Player has not selected an answer - showing warning');
        // Could show a warning message here instead of auto-submitting with 0 points
      }
      return;
    }
  ...
  }, [timeLeft, locked, showResults, selected]);
