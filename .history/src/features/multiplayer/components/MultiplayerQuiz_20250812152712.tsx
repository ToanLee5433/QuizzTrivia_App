...
export const MultiplayerQuiz: React.FC<MultiplayerQuizProps> = ({
  ...
  const [gamePhase, setGamePhase] = useState<'question' | 'results' | 'waiting' | 'finished'>('question');
  ...
  const calculatePoints = (timeRemaining: number, isCorrect: boolean) => {
    if (!isCorrect) return 0;
    const basePoints = 100;
    const timeBonus = Math.floor((timeRemaining / room.timePerQuestion) * 50);
