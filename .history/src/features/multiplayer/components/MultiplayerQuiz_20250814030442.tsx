...
const MultiplayerQuiz: React.FC<MultiplayerQuizProps> = ({
    ...
    const unsubscribe = onSnapshot(roomRef, (doc) => {
      if (doc.exists()) {
        ...
        if (newIndex !== previousQuestionIndexRef.current) {
          ...
          // Reset current question answers for new question
  ...
  useEffect(() => {
    const saveFinalResults = async () => {
      if (currentGamePhase === 'finished' && currentUser?.uid && currentRoomData?.id) {
        ...
        try {
          ...
          //   correctAnswers,
          //   totalQuestions: processedQuestions.length,
          //   accuracy: Math.round((correctAnswers/processedQuestions.length)*100),
          //   answers: myAnswers,
    ...
    saveFinalResults();
  }, [currentGamePhase, currentUser?.uid, currentRoomData?.id, playerAnswers, playerScores, processedQuestions.length]);
