...
const MultiplayerQuiz: React.FC<MultiplayerQuizProps> = ({
  ...
  const [currentQuestionAnswers, setCurrentQuestionAnswers] = useState<{[playerId: string]: {answer: number, timeSpent: number, isCorrect: boolean}}>({});
            ...
            <div className="space-y-2">
              ...
              <div className="text-center mb-4">
                <p className="text-sm text-gray-600">
                  Correct Answer: <span className="font-bold text-green-600">{questionResults.correctAnswer}</span>
                </p>
                {questionResults.explanation && (
                  <p className="text-xs text-gray-500 mt-1">{questionResults.explanation}</p>
                )}
              </div>
              
              {/* Player results will be shown when available */}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            <div className={`font-semibold ${isCurrentUser ? 'text-blue-700' : 'text-gray-800'}`}>
                              #{position} {player.username || player.name}
                              {isCurrentUser && <span className="ml-2 text-sm">(You)</span>}
                            </div>
                            <div className="text-sm text-gray-600">
                              {player.answers?.filter((a: any) => a.isCorrect).length || 0} correct answers
                            </div>
                          </div>
                        </div>
