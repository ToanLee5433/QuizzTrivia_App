...
const MultiplayerQuiz: React.FC<MultiplayerQuizProps> = ({
  ...
  useEffect(() => {
    ...
    if (nextQuestionCountdown <= 0) {
      ...
      // Only advance if this is the last question or host
      if (multiplayerService && state.roomId) {
        const isLastQuestion = currentQuestionIndex >= questions.length - 1;
        if (isLastQuestion) {
          console.log('🏁 Game finished');
          // Game will be finished by service
        } else {
          console.log('➡️ Advancing to next question');
          multiplayerService.advanceToNextQuestion(state.roomId).catch(console.error);
        }
      }
      return;
    }
  ...
  }, [nextQuestionCountdown, multiplayerService, currentQuestionIndex, questions.length]);
