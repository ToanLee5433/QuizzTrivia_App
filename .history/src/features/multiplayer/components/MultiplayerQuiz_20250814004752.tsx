...
import  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-blue-100 p-2 sm:p-4 lg:p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            ...
            <div className="text-sm text-yellow-700 space-y-1">
              <div>Questions available: {questions.length}</div>
              <div>Current index: {currentQuestionIndex}</div>
              <div>Has gameData: {!!gameData ? 'Yes' : 'No'}</div>
              <div>GameData keys: {gameData ? Object.keys(gameData).join(', ') : 'None'}</div>
              <div>Question ID: {question?.id || 'None'}</div>
              <div>Question title: {question?.title || 'None'}</div>
        ...
        {/* Quiz Header */}
        <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 mb-4 sm:mb-6">ock, Send, Users, CheckCircle, AlertCircle } from 'lucide-react';
