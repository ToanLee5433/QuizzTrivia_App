...
export class FirestoreMultiplayerService extends SimpleEventEmitter implements MultiplayerServiceInterface {
  ...
  async startGame(roomId: string): Promise<void> {
    try {
      ...
      // Set room to "starting" phase with 5-second countdown
      const gameStartTime = new Date(Date.now() + 5000); // 5 seconds from now
      await updateDoc(roomRef, {
        status: 'starting',
        gameStartAt: gameStartTime
      });

      // Wait 5 seconds then actually start the game
      setTimeout(() => {
    ...
    try {
      ...
      if (allAnswered && players.length > 0) {
        ...
        await updateDoc(roomRef, {
          ...
          'gameData.nextQuestionAt': new Date(Date.now() + 5000), // 5 seconds from now
