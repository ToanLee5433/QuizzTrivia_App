  ...
  async createRoom(roomConfig: Partial<Room>, selectedQuiz?: any): Promise<{ room: Room; player: Player }> {
    try {
      ...
      const roomData = {
        ...
        settings: {
          timePerQuestion: roomConfig.settings?.timePerQuestion
            ?? roomConfig.settings?.timeLimit
            ?? 30,
        ...
        try {
          ...
          if (quizSnap.exists()) {
            ...
            if (Array.isArray(quizData.questions) && quizData.questions.length > 0) {
              ...
              console.log('Loaded quiz questions from Firestore:', questions.length);
            }
          }
        } catch (e) {
          console.warn('Failed to load quiz by quizId, will fallback to mock:', e);
        }
      }
  ...
  private async checkAndAdvanceQuestion(roomId: string, questionId: string): Promise<void> {
    try {
      ...
      if (allAnswered && players.length > 0) {
        console.log('🚀 All players answered, advancing to next question in 3 seconds...');
  ...
  async advanceToNextQuestion(roomId: string): Promise<void> {
    try {
      ...
      this.emit('game:next-question', {
        ...
        questions,
        timePerQuestion: data.settings?.timePerQuestion || 30,
        total: questions.length,
        index: nextIndex + 1,
